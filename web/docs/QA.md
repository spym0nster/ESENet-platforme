...
- [ ] Opportunity detail shows the timeframe when set: "start → end", or "Starts <date>" / "Until <date>" with only one; hidden when neither is set (verified in-browser with a temp start/end that was reset to null)
- [ ] `/opportunities` paginates at 20 per page; filters/sort survive across Previous/Next
- [ ] Sort has three options — Newest, Starting soonest, Application deadline (`?sort=deadline` orders by `application_deadline` asc, nulls last); a card whose deadline has passed shows a muted "Applications closed" label, one within 7 days shows the red "Closes <date>" badge
...
- [ ] Verified both 404 paths in-browser (unknown route + bad opportunity id)
